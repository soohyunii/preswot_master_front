<template>
  <div id="class_index_wrapper" class="bt-container">
    <h2 class="page-title">수강과목 목록</h2>

    <el-table class="elTable-label" :data="page" style="width: 100%">
...
export default {
  ...
  async created() {
    ...
    // 검색기능 : 서버에서 DB 쿼리로 처리하는 게 효율이 나을 것 같으면 나중에 수정.
  ...
  methods: {
    ...
    async onClick(type, arg) {
      ...
      switch (type) {
        ...
        case 'SEARCH': {
          vm.$router.push({ path: '/student/NNNclass', query: { type: `${vm.searchType}`, text: `${vm.searchText}` } });
          break;
        }
        case 'CHANGE': {
          vm.page = [];
