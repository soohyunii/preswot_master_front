  ...
  <div id="teacher_class_new_wrapper" class="bt-container">
    ...
    <el-form :model="input" ref="elForm" label-position="left" label-width="125px" style="max-width: 800px;" class="elForm-label">
      ...
      <el-form-item label="강사선택">
        <select id="teacher-choice" v-model="input.choiceTeacher">
          <option value="">강사리스트</option>  <!-- DB에 있는 강사 리스트 가져오기 --> 
        </select>
      </el-form-item>
      ...
      <el-form-item label="과목차수">
        <el-input v-model="input.frequency" class="subject-title"></el-input>
      </el-form-item>

      <el-form-item label="강의시간">
        <el-input v-model="input.time" class="subject-title"></el-input>
      </el-form-item>

      <el-form-item label="강의공간">
        <el-input v-model="input.classRoom" class="subject-title"></el-input>
      </el-form-item>

      <br/><br/><br/><br/><br/>
      <el-form-item label="강의 기간" style="margin-top:50px;">
        <el-date-picker
    ...
    onSubmit() {
      ...
      vm.$refs.elForm.validate(async (/* valid, fields */) => {
        ...
        if (vm.isEdit) {
          ...
          try {
            await classService.NNMasterputClass({
              id,
              ...vm.input,
            });
            vm.$router.push('/view/class');
          } catch (error) {
            vm.$notify({
              title: '과목 수정 실패',
          ...
          // TODO: wrap with try catch
          try {
            await classService.NNMasterpostClass(vm.input);
            if(vm.input.choiceUni==''||vm.input.choiceDept==''||vm.input.code==''){
              vm.$notify({
                title: '과목 등록 실패',
              ...
              console.log('1111');
              vm.$router.push('/register/class/success');
              console.log('2222');
            }
          } catch (error) {
            vm.$notify({
