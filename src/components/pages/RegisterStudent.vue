    ...
    onSubmit() {
      ...
      vm.$refs.elForm.validate(async (/* valid, fields */) => {
        ...
        if (vm.isEdit) {
          ...
          try {
            await classService.NNMasterputStudent({
              id,
              ...vm.input,
            });
            vm.$router.push('/view/student');
          } catch (error) {
            vm.$notify({
              title: '학생 수정 실패',
          ...
          // TODO: wrap with try catch
          try {
            await classService.NNMasterpostStudent(vm.input);
            if(vm.input.email==''||vm.input.password==''||vm.input.passwordConfirm==''||vm.input.name==''){
              ...
              // console.log('@@@1');
              vm.$router.push('/register/student/success');
              // console.log('@@@2');
            }
          } catch (error) {
            vm.$notify({
