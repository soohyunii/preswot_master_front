<template>
  <div class="ts">
    ...
    <el-row :gutter="20" id="student_score">
      <el-col :span="3" v-for="(item, index) in localDataSet" :key="index">
        <div class="one_user"  v-on:click="select($event, item.user_id)">
          ...
          -->
          <div class="user_pic" :style="{ backgroundImage: ('url('+require('@/assets/profi.png')+')') }"></div>
          <div class="user_info">
            <div class="user_name">{{item.name}}</div>
            <div class="participation_score">참여도: {{(item.participation * 100).toFixed(2)}}</div>
            <!--
            <div class="concentration_score">집중도: {{item.concentration_score}}</div>
            -->
            <div class="concentration_score">이해도: {{(item.understanding * 100).toFixed(2)}}</div>
            <!--
...
<script>
  export default {
    ...
    props: ['propDataSet', 'childs-close-event'],
    data() {
      return {
        loopInterval: 0,
        localDataSet: [],
        ...
        switch (type) {
          ...
          */
          ...
          case 'GET_LECTURE_SCORE_ORDER_BY_UNDERSTANDING': {
            vm.localDataSet.sort((a, b) => {
              if (a.understanding > b.understanding) {
                return -1;
              }
              if (a.understanding < b.understanding) {
                return 1;
    ...
    mounted() {
      const vm = this;
      vm.localDataSet = vm.propDataSet;
    },
  };
</script>
