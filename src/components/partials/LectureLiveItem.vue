<template>
  <div class="lecture-live-item-wrapper" style="background: #e5e9f2">
    <div v-if="data === undefined && type !== 'TEACHER'"> <!-- 입력값 없음 -->
      강사님의 신호를 기다리는 중입니다.
    </div>
    <div v-else class="lecture-item">
      <div v-if="data.type === 0" class="question-box"> <!-- 질문 -->
        <div v-if="data.questions[0].type === 0">
          <template v-if="(studentAnswerLogIndex !== -1 && type === 'STUDENT' && data.result === 1)">
            <!-- 이미 제출한 경우 -->
            <p>제출이 완료되었습니다.</p>
            <br />
          ...
          <template v-else>
            <p>문항 - 객관</p>
            <p v-if="(studentAnswerLogIndex !== -1 || answerSubmitted) && type === 'STUDENT'" style="color: red;">제출 완료</p>
        ...
        <el-row type="flex" justify="center">
          <el-button
            v-if="!((studentAnswerLogIndex !== -1 && type === 'STUDENT' && data.result === 1) || (data.result === 1 && answerSubmitted))"
            type="primary"
            @click="preOnClick()">
            제출
          </el-button>
      ...
      <div v-if="data.type === 1">
        <div v-if="data.surveys[0].type === 0">
          <template v-if="(data.surveys[0].student_surveys.length > 0 && type === 'STUDENT' && data.result === 1)">
            <!-- 이미 제출한 경우 -->
            <p>제출이 완료되었습니다.</p>
          </template>
          <template v-else>
            <p>설문 - 객관</p>
            <p v-if="(data.surveys[0].student_surveys.length > 0 || answerSubmitted) && type === 'STUDENT'" style="color: red;">{{ studentAnswerLogIndex }}제출 완료</p>
        ...
        <el-row type="flex" justify="center">
          <el-button
            v-if="!((data.surveys[0].student_surveys.length > 0 && type === 'STUDENT' && data.result === 1) || (data.result === 1 && answerSubmitted))"
            type="primary"
            @click="preOnClick()">
            제출
          </el-button>
